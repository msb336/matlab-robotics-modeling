...
A = @(x)sin(pi/2 - phi(x));
B = @(x)(a4/a1)+cos(pi/2 - phi(x));
C = @(x)(a4/a3)*cos(pi/2 - phi(x))+ (a1^2-a2^2+a3^2+a4^2)/(2*a1*a3);
psi = @(x)2*atan((A(x)- sqrt(A(x)^2+B(x)^2-C(x)^2))/(B(x)+C(x)));

T{connections(2)} = @(x)T{connections(1)}(x)*rotate(phi(x), 'thx')*translate(oa);
T{connections(3)} = @(x)T{connections(4)}(x)*rotate(-psi(x)-sin(psi(x)), 'thx')*translate(-bc);
